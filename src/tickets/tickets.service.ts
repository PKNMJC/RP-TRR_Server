...
export class TicketsService {
  ...
  private async generateTicketCode(): Promise<string> {
    ...
    let sequence = 1;
    if (lastTicket) {
      // Extract sequence number from last ticket code (IT-2025-0001 -> 0001)
      const lastSeq = lastTicket.ticketCode.split('-')[2];
      sequence = parseInt(lastSeq) + 1;
    }

    const ticketNumber = sequence.toString().padStart(4, '0');
  ...
  async updateTicketStatus(ticketId: number, status: any, updatedBy: number, comment?: string) {
